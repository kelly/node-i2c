const wire = require('../build/Release/i2c');
const { EventEmitter } = require('events');
const tick = setImmediate || process.nextTick;

class i2c extends EventEmitter {

  constructor(address, options) {
    super();
    this.address = address;
    this.options = {
      debug: false,
      device: "/dev/i2c-1",
      ...(options || {})
    };
    this.history = [];

    if (this.options.debug) {
      require('repl').start({
        prompt: "i2c > "
      }).context.wire = this;
      process.stdin.emit('data', ''); // trigger repl
    }

    process.on('exit', () => this.close());

    this.on('data', (data) => {
      this.history.push(data);
    });

    this.on('error', (err) => {
      console.log(`Error: ${err}`);
    });

    this.open(this.options.device, (err) => {
      if (!err) {
        this.setAddress(this.address);
      }
    });
  }

  scan(callback) {
    wire.scan((err, data) => {
      tick(() => {
        // data is an Array from C++
        callback(err, data.filter((num) => num >= 0));
      });
    });
  }

  setAddress(address) {
    wire.setAddress(address);
    this.address = address;
  }

  open(device, callback) {
    wire.open(device, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }

  close() {
    wire.close();
  }

  write(buf, callback) {
    this.setAddress(this.address);
    if (!Buffer.isBuffer(buf)) {
      buf = Buffer.from(buf);
    }
    wire.write(buf, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }

  writeByte(byte, callback) {
    this.setAddress(this.address);
    wire.writeByte(byte, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }

  writeBytes(cmd, buf, callback) {
    this.setAddress(this.address);
    if (!Buffer.isBuffer(buf)) {
      buf = Buffer.from(buf);
    }
    wire.writeBlock(cmd, buf, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }

  read(len, callback) {
    this.setAddress(this.address);
    wire.read(len, (err, data) => {
      tick(() => {
        if (callback) callback(err, data);
      });
    });
  }

  readByte(callback) {
    this.setAddress(this.address);
    wire.readByte((err, data) => {
      tick(() => {
        if (callback) callback(err, data);
      });
    });
  }

  readBytes(cmd, len, callback) {
    this.setAddress(this.address);
    wire.readBlock(cmd, len, null, (err, actualBuffer) => {
      tick(() => {
        if (callback) callback(err, actualBuffer);
      });
    });
  }

  stream(cmd, len, delay) {
    if (delay == null) delay = 100;
    this.setAddress(this.address);
    wire.readBlock(cmd, len, delay, (err, data) => {
      if (err) {
        this.emit('error', err);
      } else {
        this.emit('data', {
          address: this.address,
          data: data,
          cmd: cmd,
          length: len,
          timestamp: Date.now()
        });
      }
    });
  }

  writeWord(cmd, word, callback) {
    this.setAddress(this.address);
    wire.writeWord(cmd, word, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }

  readWord(cmd, callback) {
    this.setAddress(this.address);
    wire.readWord(cmd, (err, data) => {
      tick(() => {
        if (callback) callback(err, data);
      });
    });
  }

  readProcessCall(cmd, word, callback) {
    this.setAddress(this.address);
    wire.readProcessCall(cmd, word, (err, data) => {
      tick(() => {
        if (callback) callback(err, data);
      });
    });
  }

  writeQuick(bit, callback) {
    this.setAddress(this.address);
    wire.writeQuick(bit, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }

  readByteData(cmd, callback) {
    this.setAddress(this.address);
    wire.readByteData(cmd, (err, data) => {
      tick(() => {
        if (callback) callback(err, data);
      });
    });
  }

  writeByteData(cmd, byte, callback) {
    this.setAddress(this.address);
    wire.writeByteData(cmd, byte, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }

  readBlockData(cmd, callback) {
    this.setAddress(this.address);
    wire.readBlockData(cmd, (err, data) => {
      tick(() => {
        if (callback) callback(err, data);
      });
    });
  }

  writeBlockData(cmd, buf, callback) {
    this.setAddress(this.address);
    if (!Buffer.isBuffer(buf)) {
      buf = Buffer.from(buf);
    }
    wire.writeBlockData(cmd, buf, (err) => {
      tick(() => {
        if (callback) callback(err);
      });
    });
  }
}

module.exports = i2c;
